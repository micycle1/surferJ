...
public class QuadraticSolver {
     ...
     *         - {root} if one real root (multiplicity 2).
     *         - {root1, root2} if two distinct real roots, sorted root1 <= root2.
     *         Returns roots >= -tolerance.
     */
    public static double[] solve(Polynomial p) {
        ...
        if (discriminant < -tolerance) { // Allow small negatives due to precision
            ...
            if (root1 >= -tolerance) validRoots.add(Math.max(0.0, root1));
            if (root2 >= -tolerance) validRoots.add(Math.max(0.0, root2));
