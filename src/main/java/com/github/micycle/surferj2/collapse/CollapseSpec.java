...
public class CollapseSpec implements Comparable<CollapseSpec> {
    ...
    public CollapseSpec(CollapseType type, double time, KineticTriangle triangle, int relevantEdge, double secondaryKey) {
        ...
        if (type == CollapseType.NEVER || type == CollapseType.UNDEFINED) {
            if (!Double.isNaN(this.time)) {
                //System.err.println("Warning: Time should be NaN for NEVER/UNDEFINED CollapseSpec, but was " + this.time);
            }
        }
    }
    ...
    // Simplified constructors
     ...
     public CollapseSpec(CollapseType type, double time, KineticTriangle triangle, int relevantEdge) {
        this(type, time, triangle, relevantEdge, Double.NaN);
         if (!requiresRelevantEdge(type)) {
            // Allow edge for types that *might* use it later, but warn if strictly not needed?
            //System.err.println("Warning: Relevant edge provided for CollapseType " + type + " which might not use it.");
        }
    ...
    // --- Getters ---
    public CollapseType getType() { return type; }
    public double getTime() { return time; }
    public KineticTriangle getTriangle() { return triangle; }
    public int getRelevantEdge() { return relevantEdge; }
    public double getSecondaryKey() { return secondaryKey; }


    // --- Helper Methods ---
    private static boolean requiresRelevantEdge(CollapseType type) {
        return type == CollapseType.CONSTRAINT_COLLAPSE ||
    ...
    private static boolean requiresSecondaryKey(CollapseType type) {
        return type == CollapseType.VERTEX_MOVES_OVER_SPOKE ||
               type == CollapseType.FACE_HAS_INFINITELY_FAST_VERTEX_WEIGHTED;
    }
    ...
    public int compareTo(CollapseSpec other) {
        ...
        if (this.type == CollapseType.NEVER && other.type == CollapseType.NEVER) return 0;
        if (this.type == CollapseType.NEVER) return 1; // this is later
        if (other.type == CollapseType.NEVER) return -1; // other is later
        ...
        // If times are effectively equal, compare by type priority (lower enum ordinal = higher priority)
        int typeCompare = Integer.compare(this.type.ordinal(), other.type.ordinal());
        if (typeCompare != 0) {
            return typeCompare;
        }

        // If times and types are equal, use secondary key for tie-breaking if applicable
        // Higher secondary key means HIGHER priority (comes earlier) -> reverse comparison
    ...
    public String toString() {
        ...
        sb.append(type);
        if (triangle != null) sb.append(", tri=").append(triangle.id);
        if (!Double.isNaN(time)) sb.append(", time=").append(String.format("%.6f", time));
        if (relevantEdge != -1) sb.append(", edge=").append(relevantEdge);
        if (!Double.isNaN(secondaryKey)) sb.append(", key=").append(String.format("%.4f", secondaryKey));
        sb.append('}');
        return sb.toString();
    }
    ...
    @Override
    public int hashCode() {
         // Hash based on fields relevant to priority, excluding secondary key due to tolerance issues
